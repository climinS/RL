...
class Agent(object):
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        if(DDQN==0):
            values_next = self.__target(next_batch.float()).max(1).values.detach()  #DQN
        else:
            max_action = self.__policy(next_batch.float()).max(1).indices   #DDQN
            values_next = self.__target(next_batch.float()).gather(1, max_action.unsqueeze(1)).squeeze(1)  #DDQN
